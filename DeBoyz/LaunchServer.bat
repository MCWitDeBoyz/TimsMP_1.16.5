...
echo Launching forge-1.16.5-36.2.34.jar with '%mem%' max memory, jvm args '%jvmargs%' and arguments '%launchargs%'

:: add nogui to the end of this line to disable the gui ::
%javapath% -Xmx%mem% %jvm_args%  -Dlog4j2.formatMsgNoLookups=true -Dlog4j.configurationFile=log4j2.xml -jar forge-1.16.5-36.2.34.jar %launchargs% -nogui
PAUSE
